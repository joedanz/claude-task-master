...
Important: Your response must be valid JSON only, with no additional explanation or comments.`;
			...
			const streamingTracker = (content, chunkInfo = {}) => {
				...
				if (calculatedPercent > percentComplete) {
					...
					log(
						'debug',
						`Progress update: ${prevPercent}% → ${percentComplete}%`
					);
				}
				...
				if (
					elapsedSinceLastMicroUpdate >= streamingTracker.microUpdateInterval
				) {
					...
					if (chunk.type === 'content_block_delta' && chunk.delta.text) {
						...
						if (
							...
							if (chunk.delta.usage.output_tokens) {
								chunkMetadata.completionTokens =
						...
						log(
							'debug',
							`Received message_stop event: ${JSON.stringify(chunk)}`
						);
