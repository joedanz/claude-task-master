...
export class PrdParseTracker {
  ...
  update(data) {
    ...
    if (data.percent !== undefined) {
      this.stats.current = Math.min(Math.floor((data.percent / 100) * this.stats.total), this.stats.total);
  ...
  finish(success = true, stats = {}, summaryCallback = null) {
    ...
    if (this.spinner) {
      ...
      if (success) {
        this.spinner.succeed(chalk.green(`PRD parsing complete! Generated ${this.stats.taskCount || 0} tasks`));
      } else {
    ...
    if (summaryCallback && typeof summaryCallback === 'function') {
      summaryCallback({
        ...
        taskCategories: this.stats.taskCategories || { high: 0, medium: 0, low: 0 },
  ...
  createStreamingTracker(numTasksHint = 0) {
    ...
    function calculateTokens(text) {
      ...
      const rawTotal = Math.ceil(wordTokens + digitTokens + specialChars - jsonBias);
    ...
    return (content, chunkInfo = {}) => {
      ...
      const tokenBased = Math.min(99, Math.floor((tokenCount / estimatedTotalTokens) * 100));
      const taskBased = seenTaskIds.size > 0 ? Math.min(90, Math.floor((seenTaskIds.size / numTasks) * 100)) : 0;
      const calculated = Math.max(tokenBased * 0.5 + taskBased * 0.5, percentComplete);
      percentComplete = Math.min(99, Math.round(calculated));

      // Thinking message – basic phase reporting
      const thinkingMsg = seenTaskIds.size === 0 ? 'Analyzing PRD...' : percentComplete < 90 ? `Defining task ${seenTaskIds.size + 1}...` : 'Finalizing...';
      this.updateThinkingState({ message: thinkingMsg, state: percentComplete < 90 ? 'processing' : 'finalizing' });
